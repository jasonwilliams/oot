...
#[no_mangle]
pub unsafe extern "C" fn osStopTimer(mut timer: *mut OSTimer) -> s32 {
    let mut prevInt: u32_0 = 0;
    let mut next: *mut OSTimer = 0 as *mut OSTimer;
    if (*timer).next.is_null() { return -(1 as libc::c_int) }
    prevInt = __osDisableInt() as u32_0;
    next = (*timer).next;
    if next != __osTimerList {
        (*next).value =
            ((*next).value as libc::c_ulonglong).wrapping_add((*timer).value)
                as OSTime as OSTime
    }
