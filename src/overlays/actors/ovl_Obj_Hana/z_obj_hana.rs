...
#[no_mangle]
pub static mut Obj_Hana_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id: ACTOR_OBJ_HANA as libc::c_int as s16,
                          ...
                          destroy:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(ObjHana_Destroy
                                                                          as
                                                                          unsafe extern "C" fn(_:
                                                                                                   *mut Actor,
                                                                                               _:
...
// size = 0x10
static mut sHanaParams: [HanaParams; 3] =
    unsafe {
        [{
             let mut init =
                 HanaParams{dList: gHanaDL.as_ptr() as *mut _,
                            scale: 0.01f32,
                            yOffset: 0.0f32,
                            radius: -(1 as libc::c_int) as s16,
                            height: 0 as libc::c_int as s16,};
             init
         },
         {
             let mut init =
                 HanaParams{dList: gFieldKakeraDL.as_ptr() as *mut _,
                            scale: 0.1f32,
                            yOffset: 58.0f32,
                            radius: 10 as libc::c_int as s16,
                            height: 18 as libc::c_int as s16,};
             init
         },
         {
             let mut init =
                 HanaParams{dList: gFieldBushDL.as_ptr() as *mut _,
                            scale: 0.4f32,
                            yOffset: 0.0f32,
                            radius: 12 as libc::c_int as s16,
                            height: 44 as libc::c_int as s16,};
             init
         }]
    };
...
#[no_mangle]
pub unsafe extern "C" fn ObjHana_Init(mut thisx: *mut Actor,
                                      mut globalCtx: *mut GlobalContext) {
    let mut this: *mut ObjHana = thisx as *mut ObjHana;
    let mut type_0: s16 =
        ((*this).actor.params as libc::c_int & 3 as libc::c_int) as s16;
    let mut params: *mut HanaParams =
        &mut *sHanaParams.as_mut_ptr().offset(type_0 as isize) as
            *mut HanaParams;
    Actor_ProcessInitChain(&mut (*this).actor, sInitChain.as_mut_ptr());
    Actor_SetScale(&mut (*this).actor, (*params).scale);
    (*this).actor.shape.yOffset = (*params).yOffset;
    if (*params).radius as libc::c_int >= 0 as libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ObjHana_Destroy(mut thisx: *mut Actor,
                                         mut globalCtx: *mut GlobalContext) {
    let mut this: *mut ObjHana = thisx as *mut ObjHana;
    if sHanaParams[((*this).actor.params as libc::c_int & 3 as libc::c_int) as
                       usize].radius as libc::c_int >= 0 as libc::c_int {
        Collider_DestroyCylinder(globalCtx, &mut (*this).collider);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ObjHana_Update(mut thisx: *mut Actor,
                                        mut globalCtx: *mut GlobalContext) {
    let mut this: *mut ObjHana = thisx as *mut ObjHana;
    if sHanaParams[((*this).actor.params as libc::c_int & 3 as libc::c_int) as
                       usize].radius as libc::c_int >= 0 as libc::c_int &&
...
#[no_mangle]
pub unsafe extern "C" fn ObjHana_Draw(mut thisx: *mut Actor,
                                      mut globalCtx: *mut GlobalContext) {
    Gfx_DrawDListOpa(globalCtx,
                     sHanaParams[((*thisx).params as libc::c_int &
                                      3 as libc::c_int) as usize].dList);
}
