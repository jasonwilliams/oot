...
#[no_mangle]
pub unsafe extern "C" fn Path_GetByIndex(mut globalCtx: *mut GlobalContext,
                                         mut index: s16, mut max: s16)
 -> *mut Path {
    let mut path: *mut Path = 0 as *mut Path;
    if index as libc::c_int != max as libc::c_int {
        path =
            &mut *(*globalCtx).setupPathList.offset(index as isize) as
                *mut Path
    } else { path = 0 as *mut Path }
    return path;
}
#[no_mangle]
pub unsafe extern "C" fn Path_OrientAndGetDistSq(mut actor: *mut Actor,
                                                 mut path: *mut Path,
                                                 mut waypoint: s16,
                                                 mut yaw: *mut s16) -> f32_0 {
    let mut dx: f32_0 = 0.;
    let mut dz: f32_0 = 0.;
    let mut pointPos: *mut Vec3s = 0 as *mut Vec3s;
    if path.is_null() { return -1.0f64 as f32_0 }
    pointPos =
    ...
    pointPos = &mut *pointPos.offset(waypoint as isize) as *mut Vec3s;
    dx = (*pointPos).x as libc::c_int as libc::c_float - (*actor).world.pos.x;
    dz = (*pointPos).z as libc::c_int as libc::c_float - (*actor).world.pos.z;
    *yaw =
        (Math_FAtan2F(dx, dz) *
...
#[no_mangle]
pub unsafe extern "C" fn Path_CopyLastPoint(mut path: *mut Path,
                                            mut dest: *mut Vec3f) {
    let mut pointPos: *mut Vec3s = 0 as *mut Vec3s;
    if !path.is_null() {
        pointPos =
            &mut *((*gSegments.as_mut_ptr().offset((((*path).points as u32_0)
                       ...
                       *mut Vec3s).offset(((*path).count as libc::c_int -
                                               1 as libc::c_int) as isize) as
                *mut Vec3s;
        (*dest).x = (*pointPos).x as f32_0;
        (*dest).y = (*pointPos).y as f32_0;
        (*dest).z = (*pointPos).z as f32_0
    };
}
