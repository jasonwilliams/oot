...
extern "C" {
    ...
    #[no_mangle]
    fn osJamMesg(mq: *mut OSMesgQueue, mesg: OSMesg, flag: s32) -> s32;
    #[no_mangle]
    fn osPiGetCmdQueue() -> *mut OSMesgQueue;
    #[no_mangle]
    static mut __osPiDevMgr: OSMgrArgs;
}
...
#[no_mangle]
pub unsafe extern "C" fn osEPiStartDma(mut handle: *mut OSPiHandle,
                                       mut mb: *mut OSIoMesg,
                                       mut direction: s32) -> s32 {
    let mut ret: s32 = 0;
    if __osPiDevMgr.initialized == 0 { return -(1 as libc::c_int) }
    (*mb).piHandle = handle;
    if direction == 0 as libc::c_int {
        (*mb).hdr.type_0 = 0xf as libc::c_int as u16_0
    } else { (*mb).hdr.type_0 = 0x10 as libc::c_int as u16_0 }
    if (*mb).hdr.pri as libc::c_int == 1 as libc::c_int {
        ret = osJamMesg(osPiGetCmdQueue(), mb as OSMesg, 0 as libc::c_int)
    } else {
        ret = osSendMesg(osPiGetCmdQueue(), mb as OSMesg, 0 as libc::c_int)
    }
    return ret;
}
