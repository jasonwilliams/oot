...
extern "C" {
    ...
    #[no_mangle]
    static mut object_bxa_DL_0022F0: [Gfx; 0];
    #[no_mangle]
...
pub struct EnBx {
    ...
    pub colliderQuad: ColliderQuad,
}
#[no_mangle]
pub static mut En_Bx_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id: ACTOR_EN_BX as libc::c_int as s16,
                          ...
                          instanceSize:
                              ::std::mem::size_of::<EnBx>() as libc::c_ulong,
                          init:
...
#[no_mangle]
pub unsafe extern "C" fn EnBx_Init(mut thisx: *mut Actor,
                                   mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnBx = thisx as *mut EnBx;
    let mut sp48: Vec3f =
        {
            let mut init = Vec3f{x: 0.015f32, y: 0.015f32, z: 0.015f32,};
            init
        };
    let mut sp3C: Vec3f =
    ...
    (*thisx).scale.z = 0.01f32;
    (*thisx).scale.x = (*thisx).scale.z;
    (*thisx).scale.y = 0.03f32;
    (*thisx).world.pos.y = (*thisx).world.pos.y - 100.0f32;
    i = 0 as libc::c_int;
    while i < 4 as libc::c_int {
        (*this).unk_184[i as usize] = sp48;
        if i == 0 as libc::c_int {
            (*this).unk_1B4[i as usize].x =
                ((*thisx).shape.rot.x as libc::c_int - 0x4000 as libc::c_int)
                    as s16
        }
        (*this).unk_154[i as usize] = (*thisx).world.pos;
        (*this).unk_154[i as usize].y =
            (*thisx).world.pos.y +
                (i + 1 as libc::c_int) as libc::c_float * 140.0f32;
        i += 1
    }
    ...
    Collider_SetCylinder(globalCtx, &mut (*this).collider, &mut (*this).actor,
                         &mut sCylinderInit);
    Collider_InitQuad(globalCtx, &mut (*this).colliderQuad);
    Collider_SetQuad(globalCtx, &mut (*this).colliderQuad, &mut (*this).actor,
                     &mut sQuadInit);
    (*thisx).colChkInfo.mass = 0xff as libc::c_int as u8_0;
    (*this).unk_14C = 0 as libc::c_int as s16;
    (*thisx).uncullZoneDownward = 2000.0f32;
    if Flags_GetSwitch(globalCtx,
                       (*thisx).params as libc::c_int >> 8 as libc::c_int &
                           0xff as libc::c_int) != 0 {
...
#[no_mangle]
pub unsafe extern "C" fn EnBx_Destroy(mut thisx: *mut Actor,
                                      mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnBx = thisx as *mut EnBx;
    Collider_DestroyCylinder(globalCtx, &mut (*this).collider);
}
#[no_mangle]
pub unsafe extern "C" fn func_809D1D0C(mut thisx: *mut Actor,
                                       mut globalCtx: *mut GlobalContext) {
    let mut sp5C: Vec3f =
        {
            let mut init = Vec3f{x: 8000.0f32, y: 15000.0f32, z: 2500.0f32,};
            init
        };
    let mut sp50: Vec3f =
        {
            let mut init = Vec3f{x: 8000.0f32, y: 10000.0f32, z: 2500.0f32,};
            init
        };
    static mut D_809D2540: Vec3f =
        {
            let mut init = Vec3f{x: -8000.0f32, y: 15000.0f32, z: 2500.0f32,};
            init
        };
    static mut D_809D254C: Vec3f =
        {
            let mut init = Vec3f{x: -8000.0f32, y: 10000.0f32, z: 2500.0f32,};
            init
        };
    ...
    let mut this: *mut EnBx = thisx as *mut EnBx;
    Matrix_MultVec3f(&mut D_809D2540, &mut sp44);
    Matrix_MultVec3f(&mut D_809D254C, &mut sp38);
    Matrix_MultVec3f(&mut sp5C,
                     &mut *(*this).colliderQuad.dim.quad.as_mut_ptr().offset(1
                                                                                 as
                                                                                 libc::c_int
                                                                                 as
                                                                                 isize));
    Matrix_MultVec3f(&mut sp50,
                     &mut *(*this).colliderQuad.dim.quad.as_mut_ptr().offset(0
                                                                                 as
                                                                                 libc::c_int
                                                                                 as
                                                                                 isize));
    Collider_SetQuadVertices(&mut (*this).colliderQuad, &mut sp38, &mut sp44,
                             &mut *(*this).colliderQuad.dim.quad.as_mut_ptr().offset(0
                                                                                         as
                                                                                         libc::c_int
                                                                                         as
...
#[no_mangle]
pub unsafe extern "C" fn EnBx_Update(mut thisx: *mut Actor,
                                     mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnBx = thisx as *mut EnBx;
    ...
    let mut tmp32: s16 = 0;
    let mut tmp33: s32 = 0;
    if (*thisx).xzDistToPlayer <= 70.0f32 ||
           (*this).collider.base.atFlags as libc::c_int &
               (1 as libc::c_int) << 1 as libc::c_int != 0 ||
           (*this).collider.base.acFlags as libc::c_int &
               (1 as libc::c_int) << 1 as libc::c_int != 0 ||
           (*this).colliderQuad.base.atFlags as libc::c_int &
               (1 as libc::c_int) << 1 as libc::c_int != 0 {
        if (*thisx).xzDistToPlayer <= 70.0f32 ||
               &mut (*player).actor as *mut Actor == (*this).collider.base.at
               ||
               &mut (*player).actor as *mut Actor == (*this).collider.base.ac
               ||
               &mut (*player).actor as *mut Actor ==
                   (*this).colliderQuad.base.at {
            tmp33 =
            ...
            func_8002F71C(globalCtx, &mut (*this).actor, 6.0f32, tmp32,
                          6.0f32);
            (*player).invincibilityTimer = tmp33 as s8
        }
        ...
        (*this).collider.base.acFlags =
            ((*this).collider.base.acFlags as libc::c_int &
                 !((1 as libc::c_int) << 1 as libc::c_int)) as u8_0;
        (*this).colliderQuad.base.atFlags =
            ((*this).colliderQuad.base.atFlags as libc::c_int &
                 !((1 as libc::c_int) << 1 as libc::c_int)) as u8_0;
        (*this).colliderQuad.base.at = 0 as *mut Actor;
        (*this).collider.base.ac = 0 as *mut Actor;
        (*this).collider.base.at = 0 as *mut Actor;
        (*this).unk_14C = 0x14 as libc::c_int as s16
    }
    if (*this).unk_14C as libc::c_int != 0 as libc::c_int {
        (*this).unk_14C -= 1;
        i = 0 as libc::c_int;
        while i < 4 as libc::c_int {
            if ((*this).unk_14C as libc::c_int + (i << 1 as libc::c_int)) %
                   4 as libc::c_int == 0 {
                ...
                let mut yaw: s16 = 0;
                yaw = Rand_CenteredFloat(12288.0f32) as s32 as s16;
                yaw =
                    (yaw as libc::c_int + i * 0x4000 as libc::c_int +
                         0x2000 as libc::c_int) as s16;
                pos.x = Rand_CenteredFloat(5.0f32) + (*thisx).world.pos.x;
                pos.y =
                    Rand_CenteredFloat(30.0f32) + (*thisx).world.pos.y +
                        170.0f32;
                pos.z = Rand_CenteredFloat(5.0f32) + (*thisx).world.pos.z;
                EffectSsLightning_Spawn(globalCtx, &mut pos, &mut primColor,
                                        &mut envColor,
                                        230 as libc::c_int as s16, yaw,
        ...
        Audio_PlayActorSound2(thisx,
                              (0x389a as libc::c_int - 0x800 as libc::c_int)
                                  as u16_0);
    }
    ...
    if (*thisx).params as libc::c_int & 0x80 as libc::c_int != 0 {
        CollisionCheck_SetAT(globalCtx, &mut (*globalCtx).colChkCtx,
                             &mut (*this).colliderQuad.base);
    };
}
#[no_mangle]
pub unsafe extern "C" fn EnBx_Draw(mut thisx: *mut Actor,
                                   mut globalCtx: *mut GlobalContext) {
    static mut D_809D2560: [*mut libc::c_void; 3] =
        unsafe {
    ...
    Graph_OpenDisps(dispRefs.as_mut_ptr(), (*globalCtx).state.gfxCtx,
                    b"../z_en_bx.c\x00" as *const u8 as *const libc::c_char,
                    464 as libc::c_int);
        ...
        (0xdb as libc::c_int as u32_0 &
            ...
            ((0x8 as libc::c_int * 4 as libc::c_int) as u32_0 &
                 (((0x1 as libc::c_int) << 16 as libc::c_int) -
                      1 as libc::c_int) as libc::c_uint) << 0 as libc::c_int;
    (*_g_0).words.w1 =
        gSegments[((D_809D2560[((*this).actor.params as libc::c_int &
                                    0x7f as libc::c_int) as usize] as u32_0)
                       << 4 as libc::c_int >> 28 as libc::c_int) as
                      usize].wrapping_add(D_809D2560[((*this).actor.params as
                                                          libc::c_int &
                                                          0x7f as libc::c_int)
                                                         as usize] as u32_0 &
    ...
    (*_g_2).words.w1 =
        Matrix_NewMtx((*globalCtx).state.gfxCtx,
                      b"../z_en_bx.c\x00" as *const u8 as *const libc::c_char
                          as *mut libc::c_char, 478 as libc::c_int) as
            libc::c_uint;
    if (*this).actor.params as libc::c_int & 0x80 as libc::c_int != 0 {
        func_809D1D0C(&mut (*this).actor, globalCtx);
    }
    (*this).unk_14E =
        ((*this).unk_14E as libc::c_int - 0xbb8 as libc::c_int) as s16;
    (*thisx).scale.x = Math_CosS((*this).unk_14E) * 0.0075f32 + 0.015f32;
    (*thisx).scale.z = (*thisx).scale.x;
    i = 3 as libc::c_int as s16;
    while i as libc::c_int >= 0 as libc::c_int {
        let mut off: s16 = (0x2000 as libc::c_int * i as libc::c_int) as s16;
        (*this).unk_184[i as usize].x =
            Math_CosS(((*this).unk_14E as libc::c_int + off as libc::c_int) as
                          s16) * 0.0075f32 + 0.015f32;
        (*this).unk_184[i as usize].z = (*this).unk_184[i as usize].x;
        (*this).unk_1B4[i as usize].x = (*thisx).shape.rot.x;
        (*this).unk_1B4[i as usize].y = (*thisx).shape.rot.y;
        (*this).unk_1B4[i as usize].z = (*thisx).shape.rot.z;
        i -= 1
    }
    i = 0 as libc::c_int as s16;
    while (i as libc::c_int) < 4 as libc::c_int {
        Matrix_Translate((*this).unk_154[i as usize].x,
                         (*this).unk_154[i as usize].y,
                         (*this).unk_154[i as usize].z,
                         MTXMODE_NEW as libc::c_int as u8_0);
        Matrix_RotateZYX((*this).unk_1B4[i as usize].x,
                         (*this).unk_1B4[i as usize].y,
                         (*this).unk_1B4[i as usize].z,
                         MTXMODE_APPLY as libc::c_int as u8_0);
        Matrix_Scale((*this).unk_184[i as usize].x,
                     (*this).unk_184[i as usize].y,
                     (*this).unk_184[i as usize].z,
                     MTXMODE_APPLY as libc::c_int as u8_0);
        Matrix_ToMtx(mtx,
                     b"../z_en_bx.c\x00" as *const u8 as *const libc::c_char
                         as *mut libc::c_char, 507 as libc::c_int);
        i += 1;
        mtx = mtx.offset(1)
    }
    ...
    (*_g_3).words.w0 =
        (0xde as libc::c_int as u32_0 &
            ...
            (0 as libc::c_int as u32_0 &
                 (((0x1 as libc::c_int) << 16 as libc::c_int) -
                      1 as libc::c_int) as libc::c_uint) << 0 as libc::c_int;
    (*_g_3).words.w1 = object_bxa_DL_0022F0.as_mut_ptr() as libc::c_uint;
...
unsafe extern "C" fn run_static_initializers() {
    sInitChain =
        [{
             ...
             init.set_offset(&(*(0 as *const Actor as
                                     *mut Actor)).targetArrowOffset as
                                 *const f32_0 as *mut f32_0 as size_t as
                                 u32_0);
