...
#[no_mangle]
pub unsafe extern "C" fn osViSetXScale(mut value: f32_0) {
    let mut nomValue: u32_0 = 0;
    let mut prevInt: u32_0 = __osDisableInt() as u32_0;
    (*__osViNext).x.factor = value;
    (*__osViNext).state =
        ((*__osViNext).state as libc::c_int | 0x2 as libc::c_int) as u16_0;
    nomValue =
        (*(*__osViNext).modep).comRegs.xScale &
            0xfff as libc::c_int as libc::c_uint;
    (*__osViNext).x.scale =
        ((*__osViNext).x.factor * nomValue as libc::c_float) as u32_0 &
            0xfff as libc::c_int as libc::c_uint;
