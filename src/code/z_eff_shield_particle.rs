...
static mut sVertices: [Vtx; 5] =
    [Vtx{v:
     ...
     Vtx{v: Vtx_t{ob: [0; 3], flag: 0, tc: [0; 2], cn: [0; 4],},}];
// original name: "EffectShieldParticle_ct"
#[no_mangle]
pub unsafe extern "C" fn EffectShieldParticle_Init(mut thisx:
                                                       *mut libc::c_void,
                                                   mut initParamsx:
                                                       *mut libc::c_void) {
    let mut this: *mut EffectShieldParticle =
        thisx as *mut EffectShieldParticle;
    let mut initParams: *mut EffectShieldParticleInit =
        initParamsx as *mut EffectShieldParticleInit;
    let mut elem: *mut EffectShieldParticleElement =
        0 as *mut EffectShieldParticleElement;
    if !this.is_null() && !initParams.is_null() {
        (*this).numElements = (*initParams).numElements;
        if (*this).numElements as libc::c_int >
               (::std::mem::size_of::<[EffectShieldParticleElement; 16]>() as
                    libc::c_ulong).wrapping_div(::std::mem::size_of::<EffectShieldParticleElement>()
                                                    as libc::c_ulong) as s32 {
            osSyncPrintf(b"\x1b[31m\x00" as *const u8 as *const libc::c_char);
            osSyncPrintf(b"EffectShieldParticle_ct():\xe3\x83\x91\xe3\x83\xbc\xe3\x83\x86\xe3\x82\xa3\xe3\x82\xaf\xe3\x83\xab\xe6\x95\xb0\xe3\x81\x8c\xe3\x82\xaa\xe3\x83\xbc\xe3\x83\x90\xe3\x81\x97\xe3\x81\xa6\xe3\x81\xbe\xe3\x81\x99\xe3\x80\x82\n\x00"
        ...
        (*this).timer = 0 as libc::c_int as u8_0;
        elem =
            &mut *(*this).elements.as_mut_ptr().offset(0 as libc::c_int as
                                                           isize) as
                *mut EffectShieldParticleElement;
        while elem <
                  &mut *(*this).elements.as_mut_ptr().offset((*this).numElements
                                                                 as isize) as
                      *mut EffectShieldParticleElement {
            (*elem).initialSpeed =
                Rand_ZeroOne() * ((*this).maxInitialSpeed * 0.5f32) +
                    (*this).maxInitialSpeed * 0.5f32;
            (*elem).endX = 0.0f32;
            (*elem).startXChange = 0.0f32;
            (*elem).startX = 0.0f32;
            (*elem).endXChange = (*elem).initialSpeed;
            (*elem).yaw = (Rand_ZeroOne() * 65534.0f32) as s16;
            (*elem).pitch = (Rand_ZeroOne() * 65534.0f32) as s16;
            elem = elem.offset(1)
        }
        (*this).lightDecay = (*initParams).lightDecay;
        if (*this).lightDecay == 1 as libc::c_int {
            (*this).lightInfo.type_0 =
                LIGHT_POINT_NOGLOW as libc::c_int as u8_0;
            (*this).lightInfo.params.point = (*initParams).lightPoint;
            (*this).lightNode =
                LightContext_InsertLight(Effect_GetGlobalCtx(),
                                         &mut (*(Effect_GetGlobalCtx as
                                                     unsafe extern "C" fn()
                                                         ->
                                                             *mut GlobalContext)()).lightCtx,
                                         &mut (*this).lightInfo)
        } else { (*this).lightNode = 0 as *mut LightNode }
    };
}
#[no_mangle]
pub unsafe extern "C" fn EffectShieldParticle_Destroy(mut thisx:
                                                          *mut libc::c_void) {
    let mut this: *mut EffectShieldParticle =
        thisx as *mut EffectShieldParticle;
    if !this.is_null() && (*this).lightDecay == 1 as libc::c_int {
        if (*this).lightNode == (*Effect_GetGlobalCtx()).lightCtx.listHead {
            let ref mut fresh0 = (*Effect_GetGlobalCtx()).lightCtx.listHead;
            *fresh0 = (*(*this).lightNode).next
        }
        LightContext_RemoveLight(Effect_GetGlobalCtx(),
                                 &mut (*(Effect_GetGlobalCtx as
                                             unsafe extern "C" fn()
                                                 ->
                                                     *mut GlobalContext)()).lightCtx,
...
#[no_mangle]
pub unsafe extern "C" fn EffectShieldParticle_Update(mut thisx:
                                                         *mut libc::c_void)
 -> s32 {
    let mut this: *mut EffectShieldParticle =
        thisx as *mut EffectShieldParticle;
    ...
    elem =
        &mut *(*this).elements.as_mut_ptr().offset(0 as libc::c_int as isize)
            as *mut EffectShieldParticleElement;
    while elem <
              &mut *(*this).elements.as_mut_ptr().offset((*this).numElements
                                                             as isize) as
                  *mut EffectShieldParticleElement {
        (*elem).endXChange -= (*this).deceleration;
        if (*elem).endXChange < 0.0f32 { (*elem).endXChange = 0.0f32 }
        if (*elem).startXChange > 0.0f32 {
            (*elem).startXChange -= (*this).deceleration;
            if (*elem).startXChange < 0.0f32 { (*elem).startXChange = 0.0f32 }
        }
        (*elem).endX += (*elem).endXChange;
        (*elem).startX += (*elem).startXChange;
        if (*elem).startXChange == 0.0f32 &&
               (*this).lengthCutoff < (*elem).endX - (*elem).startX {
            (*elem).startXChange = (*elem).initialSpeed
        }
        elem = elem.offset(1)
    }
    if (*this).lightDecay == 1 as libc::c_int {
        (*this).lightInfo.params.point.radius =
            ((*this).lightInfo.params.point.radius as libc::c_int /
                 2 as libc::c_int) as s16
    }
    (*this).timer = (*this).timer.wrapping_add(1);
    if ((*this).duration as libc::c_int) < (*this).timer as libc::c_int {
        return 1 as libc::c_int
    }
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn EffectShieldParticle_GetColors(mut this:
                                                            *mut EffectShieldParticle,
                                                        mut primColor:
                                                            *mut Color_RGBA8,
                                                        mut envColor:
                                                            *mut Color_RGBA8) {
    let mut halfDuration: s32 =
        ((*this).duration as libc::c_int as libc::c_float * 0.5f32) as s32;
    let mut ratio: f32_0 = 0.;
    if halfDuration == 0 as libc::c_int {
    ...
    } else if ((*this).timer as libc::c_int) < halfDuration {
        ratio =
            (*this).timer as libc::c_int as libc::c_float /
                halfDuration as f32_0;
        (*primColor).r =
            ((*this).primColorStart.r as libc::c_int as libc::c_float +
                 ((*this).primColorMid.r as libc::c_int -
                      (*this).primColorStart.r as libc::c_int) as
                     libc::c_float * ratio) as u8_0;
        (*primColor).g =
            ((*this).primColorStart.g as libc::c_int as libc::c_float +
                 ((*this).primColorMid.g as libc::c_int -
                      (*this).primColorStart.g as libc::c_int) as
                     libc::c_float * ratio) as u8_0;
        (*primColor).b =
            ((*this).primColorStart.b as libc::c_int as libc::c_float +
                 ((*this).primColorMid.b as libc::c_int -
                      (*this).primColorStart.b as libc::c_int) as
                     libc::c_float * ratio) as u8_0;
        (*primColor).a =
            ((*this).primColorStart.a as libc::c_int as libc::c_float +
                 ((*this).primColorMid.a as libc::c_int -
                      (*this).primColorStart.a as libc::c_int) as
                     libc::c_float * ratio) as u8_0;
        (*envColor).r =
            ((*this).envColorStart.r as libc::c_int as libc::c_float +
                 ((*this).envColorMid.r as libc::c_int -
                      (*this).envColorStart.r as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*envColor).g =
            ((*this).envColorStart.g as libc::c_int as libc::c_float +
                 ((*this).envColorMid.g as libc::c_int -
                      (*this).envColorStart.g as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*envColor).b =
            ((*this).envColorStart.b as libc::c_int as libc::c_float +
                 ((*this).envColorMid.b as libc::c_int -
                      (*this).envColorStart.b as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*envColor).a =
            ((*this).envColorStart.a as libc::c_int as libc::c_float +
                 ((*this).envColorMid.a as libc::c_int -
                      (*this).envColorStart.a as libc::c_int) as libc::c_float
                     * ratio) as u8_0
    } else {
        ratio =
            ((*this).timer as libc::c_int - halfDuration) as libc::c_float /
                halfDuration as f32_0;
        (*primColor).r =
            ((*this).primColorMid.r as libc::c_int as libc::c_float +
                 ((*this).primColorEnd.r as libc::c_int -
                      (*this).primColorMid.r as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*primColor).g =
            ((*this).primColorMid.g as libc::c_int as libc::c_float +
                 ((*this).primColorEnd.g as libc::c_int -
                      (*this).primColorMid.g as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*primColor).b =
            ((*this).primColorMid.b as libc::c_int as libc::c_float +
                 ((*this).primColorEnd.b as libc::c_int -
                      (*this).primColorMid.b as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*primColor).a =
            ((*this).primColorMid.a as libc::c_int as libc::c_float +
                 ((*this).primColorEnd.a as libc::c_int -
                      (*this).primColorMid.a as libc::c_int) as libc::c_float
                     * ratio) as u8_0;
        (*envColor).r =
            ((*this).envColorMid.r as libc::c_int as libc::c_float +
                 ((*this).envColorEnd.r as libc::c_int -
                      (*this).envColorMid.r as libc::c_int) as libc::c_float *
                     ratio) as u8_0;
        (*envColor).g =
            ((*this).envColorMid.g as libc::c_int as libc::c_float +
                 ((*this).envColorEnd.g as libc::c_int -
                      (*this).envColorMid.g as libc::c_int) as libc::c_float *
                     ratio) as u8_0;
        (*envColor).b =
            ((*this).envColorMid.b as libc::c_int as libc::c_float +
                 ((*this).envColorEnd.b as libc::c_int -
                      (*this).envColorMid.b as libc::c_int) as libc::c_float *
                     ratio) as u8_0;
        (*envColor).a =
            ((*this).envColorMid.a as libc::c_int as libc::c_float +
                 ((*this).envColorEnd.a as libc::c_int -
                      (*this).envColorMid.a as libc::c_int) as libc::c_float *
                     ratio) as u8_0
    };
}
#[no_mangle]
pub unsafe extern "C" fn EffectShieldParticle_Draw(mut thisx:
                                                       *mut libc::c_void,
                                                   mut gfxCtx:
                                                       *mut GraphicsContext) {
    let mut this: *mut EffectShieldParticle =
    ...
    Graph_OpenDisps(dispRefs.as_mut_ptr(), gfxCtx,
                    b"../z_eff_shield_particle.c\x00" as *const u8 as
    ...
    if !this.is_null() {
            ...
            (0xfd as libc::c_int as u32_0 &
                ...
                ((1 as libc::c_int - 1 as libc::c_int) as u32_0 &
                     (((0x1 as libc::c_int) << 12 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    0 as libc::c_int;
        (*_g_2).words.w1 = gUnknownCircle6Tex.as_mut_ptr() as libc::c_uint;
            ...
            (0xd9 as libc::c_int as u32_0 &
                ...
                (!((0x600 as libc::c_int | 0x10000 as libc::c_int |
                     ...
                     (((0x1 as libc::c_int) << 24 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    0 as libc::c_int;
        (*_g_11).words.w1 = 0 as libc::c_int as u32_0;
        ...
        EffectShieldParticle_GetColors(this, &mut primColor, &mut envColor);
        ...
        while elem <
            ...
            let mut sp104: MtxF = MtxF{mf: [[0.; 4]; 4],};
            let mut spC4: MtxF = MtxF{mf: [[0.; 4]; 4],};
            let mut sp84: MtxF = MtxF{mf: [[0.; 4]; 4],};
            let mut temp1: f32_0 =
                (((*elem).endX + (*elem).startX) * 0.5f32) as s16 as f32_0;
            let mut temp2: f32_0 = (*elem).endX - (*elem).startX;
            let mut temp3: f32_0 =
                (temp2 * (1.0f32 / 64.0f32) / 0.02f32) as s16 as f32_0;
            if temp3 < 1.0f32 { temp3 = 1.0f32 }
            SkinMatrix_SetTranslate(&mut spC4, (*this).position.x as f32_0,
                                    (*this).position.y as f32_0,
                                    (*this).position.z as f32_0);
            SkinMatrix_SetRotateZYX(&mut sp104, 0 as libc::c_int as s16,
                                    (*elem).yaw, 0 as libc::c_int as s16);
            SkinMatrix_MtxFMtxFMult(&mut spC4, &mut sp104, &mut sp84);
            SkinMatrix_SetRotateZYX(&mut sp104, 0 as libc::c_int as s16,
                                    0 as libc::c_int as s16, (*elem).pitch);
            SkinMatrix_MtxFMtxFMult(&mut sp84, &mut sp104, &mut spC4);
            SkinMatrix_SetTranslate(&mut sp104, temp1, 0.0f32, 0.0f32);
            SkinMatrix_MtxFMtxFMult(&mut spC4, &mut sp104, &mut sp84);
            SkinMatrix_SetScale(&mut sp104, temp3 * 0.02f32, 0.02f32,
                                0.02f32);
            SkinMatrix_MtxFMtxFMult(&mut sp84, &mut sp104, &mut spC4);
            mtx = SkinMatrix_MtxFToNewMtx(gfxCtx, &mut spC4);
            if mtx.is_null() { break ; }
                ...
                (0xda as libc::c_int as u32_0 &
                    ...
                    (((0 as libc::c_int | 0x2 as libc::c_int |
                         ...
                         (((0x1 as libc::c_int) << 8 as libc::c_int) -
                              1 as libc::c_int) as libc::c_uint) <<
                        0 as libc::c_int;
            (*_g_16).words.w1 = mtx as libc::c_uint;
                ...
                (0x1 as libc::c_int as u32_0 &
                    ...
                    ((0 as libc::c_int + 4 as libc::c_int) as u32_0 &
                         (((0x1 as libc::c_int) << 7 as libc::c_int) -
                              1 as libc::c_int) as libc::c_uint) <<
                        1 as libc::c_int;
            (*_g_17).words.w1 = sVertices.as_mut_ptr() as libc::c_uint;
