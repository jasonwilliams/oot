...
#[no_mangle]
pub unsafe extern "C" fn func_8006BA00(mut globalCtx: *mut GlobalContext) {
    let mut sources: *mut SoundSource =
        &mut *(*globalCtx).soundSources.as_mut_ptr().offset(0 as libc::c_int
                                                                as isize) as
            *mut SoundSource;
    let mut i: s32 = 0;
    // clang-format off
    i = 0 as libc::c_int;
    while i <
              (::std::mem::size_of::<[SoundSource; 16]>() as
                   libc::c_ulong).wrapping_div(::std::mem::size_of::<SoundSource>()
                                                   as libc::c_ulong) as s32 {
        (*sources.offset(i as isize)).countdown = 0 as libc::c_int as u16_0;
        i += 1
    };
    // clang-format on
}
#[no_mangle]
pub unsafe extern "C" fn func_8006BA30(mut globalCtx: *mut GlobalContext) {
    let mut source: *mut SoundSource =
        &mut *(*globalCtx).soundSources.as_mut_ptr().offset(0 as libc::c_int
                                                                as isize) as
            *mut SoundSource;
    let mut i: s32 = 0;
    ...
    while i <
              (::std::mem::size_of::<[SoundSource; 16]>() as
                   libc::c_ulong).wrapping_div(::std::mem::size_of::<SoundSource>()
                                                   as libc::c_ulong) as s32 {
        if (*source).countdown as libc::c_int != 0 as libc::c_int {
            if (if (*source).countdown as libc::c_int == 0 as libc::c_int {
                    0 as libc::c_int
                } else {
                    (*source).countdown = (*source).countdown.wrapping_sub(1);
                    (*source).countdown as libc::c_int
                }) == 0 as libc::c_int {
                Audio_StopSfxByPos(&mut (*source).relativePos);
            } else {
                SkinMatrix_Vec3fMtxFMultXYZ(&mut (*globalCtx).viewProjectionMtxF,
                                            &mut (*source).originPos,
                                            &mut (*source).relativePos);
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn Audio_PlaySoundAtPosition(mut globalCtx:
                                                       *mut GlobalContext,
                                                   mut pos: *mut Vec3f,
                                                   mut duration: s32,
                                                   mut sfxId: u16_0) {
    let mut countdown: s32 = 0;
    let mut source: *mut SoundSource = 0 as *mut SoundSource;
    let mut smallestCountdown: s32 = 0xffff as libc::c_int;
    let mut backupSource: *mut SoundSource = 0 as *mut SoundSource;
    let mut i: s32 = 0;
    source =
        &mut *(*globalCtx).soundSources.as_mut_ptr().offset(0 as libc::c_int
                                                                as isize) as
    ...
    while i <
              (::std::mem::size_of::<[SoundSource; 16]>() as
                   libc::c_ulong).wrapping_div(::std::mem::size_of::<SoundSource>()
                                                   as libc::c_ulong) as s32 {
        if (*source).countdown as libc::c_int == 0 as libc::c_int { break ; }
        countdown = (*source).countdown as s32;
    ...
    (*source).countdown = duration as u16_0;
    SkinMatrix_Vec3fMtxFMultXYZ(&mut (*globalCtx).viewProjectionMtxF,
                                &mut (*source).originPos,
                                &mut (*source).relativePos);
    Audio_PlaySoundGeneral(sfxId, &mut (*source).relativePos,
