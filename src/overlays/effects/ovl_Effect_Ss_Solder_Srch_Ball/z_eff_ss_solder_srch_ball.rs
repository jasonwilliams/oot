...
#[no_mangle]
pub static mut Effect_Ss_Solder_Srch_Ball_InitVars: EffectSsInit =
    unsafe {
        {
            let mut init =
                EffectSsInit{type_0:
                                 EFFECT_SS_SOLDER_SRCH_BALL as libc::c_int as
                                     u32_0,
                             init:
                                 Some(EffectSsSolderSrchBall_Init as
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn EffectSsSolderSrchBall_Init(mut globalCtx:
                                                         *mut GlobalContext,
                                                     mut index: u32_0,
                                                     ...
                                                     mut initParamsx:
                                                         *mut libc::c_void)
 -> u32_0 {
    let mut initParams: *mut EffectSsSolderSrchBallInitParams =
        initParamsx as
            *mut EffectSsSolderSrchBallInitParams; // actor field was incorrectly used as a pointer to something else
    (*this).pos = (*initParams).pos;
    (*this).velocity = (*initParams).velocity;
    (*this).accel = (*initParams).accel;
    (*this).update =
        Some(EffectSsSolderSrchBall_Update as
                 unsafe extern "C" fn(_: *mut GlobalContext, _: u32_0,
                                      _: *mut EffectSs) -> ());
    (*this).life = 100 as libc::c_int as s16;
    (*this).regs[1 as libc::c_int as usize] = (*initParams).unused;
    (*this).actor = (*initParams).linkDetected as *mut Actor;
    return 1 as libc::c_int as u32_0;
}
#[no_mangle]
pub unsafe extern "C" fn EffectSsSolderSrchBall_Update(mut globalCtx:
                                                           *mut GlobalContext,
                                                       mut index: u32_0,
                                                       mut this:
                                                           *mut EffectSs) {
    let mut pad: s32 = 0;
    let mut playerPosDiffX: f32_0 = 0.;
    let mut playerPosDiffY: f32_0 = 0.;
    let mut playerPosDiffZ: f32_0 = 0.;
    let mut linkDetected: *mut s16 = 0 as *mut s16;
    let mut player: *mut Player =
        (*globalCtx).actorCtx.actorLists[ACTORCAT_PLAYER as libc::c_int as
                                             usize].head as *mut Player;
    linkDetected = (*this).actor as *mut s16;
    ...
    if BgCheck_SphVsFirstPoly(&mut (*globalCtx).colCtx, &mut (*this).pos,
                              30.0f32) == 0 {
        if sqrtf(playerPosDiffX * playerPosDiffX +
                     playerPosDiffY * playerPosDiffY +
                     playerPosDiffZ * playerPosDiffZ) < 70.0f32 {
            *linkDetected = 1 as libc::c_int as s16
        }
    } else if (*this).life as libc::c_int > 1 as libc::c_int {
