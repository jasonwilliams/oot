...
extern "C" {
    ...
    #[no_mangle]
    static mut gSlidingBladeTrapDL: [Gfx; 0];
}
...
pub struct EnTrap {
    pub actor: Actor,
    pub playerDmgTimer: s32,
    pub upperParams: s16,
    pub genericVar1: s16,
    pub genericVar2: f32_0,
    pub targetPosLeft: Vec3f,
    pub targetPosRight: Vec3f,
    pub targetPosFwd: Vec3f,
    pub targetPosBack: Vec3f,
    pub moveSpeedLeftRight: Vec3f,
    pub moveSpeedForwardBack: Vec3f,
    pub collider: ColliderCylinder,
}
#[no_mangle]
pub static mut En_Trap_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id: ACTOR_EN_TRAP as libc::c_int as s16,
                          category: ACTORCAT_BG as libc::c_int as u8_0,
                          flags:
                              ((1 as libc::c_int) << 4 as libc::c_int) as
                                  u32_0,
                          objectId: OBJECT_TRAP as libc::c_int as s16,
                          instanceSize:
                              ::std::mem::size_of::<EnTrap>() as
                                  libc::c_ulong,
                          init:
...
// Circular motion
// Four-way motion
// relative to spike trap's facing angle if moving out, absolute if moving in
#[no_mangle]
pub unsafe extern "C" fn EnTrap_Init(mut thisx: *mut Actor,
                                     mut globalCtx: *mut GlobalContext) {
    let mut trapDist: f32_0 = 0.; // required to match
    let mut trapSpeed: f32_0 = 0.; // Four-way motion
    let mut zSpeed: s16 = 0; // unused return value from function
    let mut xSpeed: s16 = 0; // unused return value from function
    let mut this: *mut EnTrap =
        thisx as *mut EnTrap; // unused return value from function
    let mut unused: *mut ColliderCylinder = &mut (*this).collider;
    (*this).upperParams =
    ...
    if (*thisx).params as libc::c_int & 0x10 as libc::c_int != 0 {
        (*this).moveSpeedForwardBack.z =
            ((*this).upperParams as libc::c_int & 0xf as libc::c_int) as
                f32_0;
        (*thisx).speedXZ = (*this).moveSpeedForwardBack.z;
        Audio_PlayActorSound2(thisx, 0x287d as libc::c_int as u16_0);
    } else if (*thisx).params as libc::c_int & 0x20 as libc::c_int != 0 {
        (*this).genericVar2 =
            ((*this).upperParams as libc::c_int & 0xf as libc::c_int) as
                libc::c_float * 40.0f32;
        (*this).upperParams =
            ((((*this).upperParams as libc::c_int & 0xf0 as libc::c_int) +
                  0x10 as libc::c_int) << 5 as libc::c_int) as s16;
        (*thisx).world.pos.x =
            (*thisx).home.pos.x +
                Math_SinS(0 as libc::c_int as s16) * (*this).genericVar2;
        (*thisx).world.pos.z =
            (*thisx).home.pos.z +
                Math_CosS(0 as libc::c_int as s16) * (*this).genericVar2
    } else {
        if (*this).upperParams as libc::c_int != 0 as libc::c_int {
            trapDist =
                (((*this).upperParams as libc::c_int >> 4 as libc::c_int) *
                     40 as libc::c_int) as f32_0;
            trapSpeed =
                ((*this).upperParams as libc::c_int & 0xf as libc::c_int) as
                    f32_0
        } else {
            trapDist = 200.0f32;
            trapSpeed = 10.0f32;
            (*thisx).params = 0xf as libc::c_int as s16
        }
        Actor_UpdateBgCheckInfo(globalCtx, thisx, 10.0f32, 20.0f32, 20.0f32,
                                0x1d as libc::c_int);
        (*thisx).home.pos = (*thisx).world.pos;
        (*this).targetPosLeft.x =
            (*thisx).world.pos.x + trapDist * Math_CosS((*thisx).world.rot.y);
        (*this).targetPosLeft.z =
            (*thisx).world.pos.z - trapDist * Math_SinS((*thisx).world.rot.y);
        (*this).targetPosRight.x =
            (*thisx).world.pos.x +
                trapDist *
                    Math_CosS(((*thisx).world.rot.y as libc::c_int +
                                   0x8000 as libc::c_int) as s16);
        (*this).targetPosRight.z =
            (*thisx).world.pos.z -
                trapDist *
                    Math_SinS(((*thisx).world.rot.y as libc::c_int +
                                   0x8000 as libc::c_int) as s16);
        (*this).targetPosFwd.x =
            (*thisx).world.pos.x + trapDist * Math_SinS((*thisx).world.rot.y);
        (*this).targetPosFwd.z =
            (*thisx).world.pos.z + trapDist * Math_CosS((*thisx).world.rot.y);
        (*this).targetPosBack.x =
            (*thisx).world.pos.x +
                trapDist *
                    Math_SinS(((*thisx).world.rot.y as libc::c_int +
                                   0x8000 as libc::c_int) as s16);
        (*this).targetPosBack.z =
            (*thisx).world.pos.z +
                trapDist *
                    Math_CosS(((*thisx).world.rot.y as libc::c_int +
                                   0x8000 as libc::c_int) as s16);
        zSpeed = (trapSpeed * Math_CosS((*thisx).world.rot.y)) as s16;
        xSpeed = (trapSpeed * Math_SinS((*thisx).world.rot.y)) as s16;
        zSpeed =
            if zSpeed as libc::c_int >= 0 as libc::c_int {
                zSpeed as libc::c_int
            } else { -(zSpeed as libc::c_int) } as s16;
        xSpeed =
            if xSpeed as libc::c_int >= 0 as libc::c_int {
                xSpeed as libc::c_int
            } else { -(xSpeed as libc::c_int) } as s16;
        (*this).moveSpeedForwardBack.z = zSpeed as f32_0;
        (*this).moveSpeedLeftRight.x = (*this).moveSpeedForwardBack.z;
        (*this).moveSpeedForwardBack.x = xSpeed as f32_0;
        (*this).moveSpeedLeftRight.z = (*this).moveSpeedForwardBack.x
    }
...
#[no_mangle]
pub unsafe extern "C" fn EnTrap_Destroy(mut thisx: *mut Actor,
                                        mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnTrap = thisx as *mut EnTrap;
    Collider_DestroyCylinder(globalCtx, &mut (*this).collider);
}
#[no_mangle]
pub unsafe extern "C" fn EnTrap_Update(mut thisx: *mut Actor,
                                       mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnTrap = thisx as *mut EnTrap;
    let mut posTemp: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut angleToKnockPlayer: s16 = 0;
    let mut angleToCollidedActor: s16 = 0;
    let mut touchingActor: s16 = 0;
    let mut blockedOnReturn: s16 = 0;
    let mut pad: s32 = 0;
    let mut angleToWall: s16 = 0;
    let mut icePos: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut posAhead: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut colPoint: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut colPoly: *mut CollisionPoly = 0 as *mut CollisionPoly;
    let mut bgId: s32 = 0;
    let mut temp_cond: f32_0 = 0.;
    touchingActor = 0 as libc::c_int as s16;
    blockedOnReturn = 0 as libc::c_int as s16;
    angleToWall =
    ...
    if (*this).collider.base.ocFlags1 as libc::c_int &
        ...
        angleToCollidedActor =
            ((*thisx).world.rot.y as libc::c_int +
                 Math_Vec3f_Yaw(&mut (*(*this).collider.base.oc).world.pos,
                                &mut (*thisx).world.pos) as libc::c_int) as
                s16;
        touchingActor = 1 as libc::c_int as s16
    }
    ...
    if (*thisx).colorFilterTimer as libc::c_int == 0 as libc::c_int {
        if (*this).playerDmgTimer == 0 as libc::c_int {
        } else { (*this).playerDmgTimer -= 1 };
        ...
        if (*thisx).xzDistToPlayer <= 40.0f32 &&
               (*this).playerDmgTimer == 0 as libc::c_int &&
               (*thisx).yDistToPlayer <= 20.0f32 {
            if (*thisx).params as libc::c_int &
                   (0x10 as libc::c_int | 0x20 as libc::c_int) == 0 {
                // if in 4-way mode:
                if ((*this).genericVar1 as libc::c_int -
                        (*thisx).yawTowardsPlayer as libc::c_int) as s16 as
                       libc::c_int >= 0 as libc::c_int {
                    angleToKnockPlayer =
                        ((*this).genericVar1 as libc::c_int -
            ...
            } else { angleToKnockPlayer = (*thisx).yawTowardsPlayer }
            (*globalCtx).damagePlayer.expect("non-null function pointer")(globalCtx,
                                                                          -(4
                                                                                as
                                                                                libc::c_int));
            func_8002F7A0(globalCtx, thisx, 6.0f32, angleToKnockPlayer,
                          6.0f32);
            (*this).playerDmgTimer = 15 as libc::c_int
        }
        if (*thisx).params as libc::c_int & 0x10 as libc::c_int != 0 {
            (*this).genericVar2 = 1.0f32;
            ...
            if touchingActor as libc::c_int != 0 &&
                   (*this).genericVar2 != 0.0f32 {
                angleToCollidedActor =
                    (Math_Vec3f_Yaw(&mut (*thisx).world.pos,
                                    &mut (*(*this).collider.base.oc).world.pos)
                         as libc::c_int - (*thisx).world.rot.y as libc::c_int)
                        as s16;
                if (if angleToCollidedActor as libc::c_int >= 0 as libc::c_int
                       {
                        angleToCollidedActor as libc::c_int
                    } else { -(angleToCollidedActor as libc::c_int) }) <
            ...
            if (*this).genericVar2 == 0.0f32 {
                (*thisx).world.rot.y =
                    ((*thisx).world.rot.y as libc::c_int +
                         0x8000 as libc::c_int) as s16; // 4 way movement
                Audio_PlayActorSound2(thisx, 0x287d as libc::c_int as u16_0);
            }
        } else if (*thisx).params as libc::c_int & 0x20 as libc::c_int != 0 {
            temp_cond = Math_SinS((*this).genericVar1);
            (*this).genericVar1 =
                ((*this).genericVar1 as libc::c_int +
                     (*this).upperParams as libc::c_int) as s16;
            // Every full circle make a sound:
            if temp_cond < 0.0f32 && Math_SinS((*this).genericVar1) >= 0.0f32
               {
                Audio_PlayActorSound2(thisx, 0x28d2 as libc::c_int as u16_0);
            }
            (*thisx).world.pos.x =
                (*this).genericVar2 * Math_SinS((*this).genericVar1) +
                    (*thisx).home.pos.x;
            (*thisx).world.pos.z =
                (*this).genericVar2 * Math_CosS((*this).genericVar1) +
                    (*thisx).home.pos.z;
            (*thisx).world.pos.y = (*thisx).floorHeight;
            (*thisx).prevPos = (*thisx).world.pos
        } else if (*this).genericVar2 != 0.0f32 {
            let mut current_block_117: u64;
            match (*this).genericVar1 as libc::c_int {
                0 => {
                    // if moving outwards:
                    // movement direction relative to spike trap
                    if (*thisx).params as libc::c_int &
                           (1 as libc::c_int) << 0 as libc::c_int == 0 {
                        (*this).genericVar2 = 0.0f32
                    } else if (*thisx).bgCheckFlags as libc::c_int &
                                  8 as libc::c_int != 0 &&
                                  (if angleToWall as libc::c_int >=
                                          0 as libc::c_int {
                                       angleToWall as libc::c_int
                                   } else { -(angleToWall as libc::c_int) }) >
                                      0x6000 as libc::c_int {
                    ...
                    if touchingActor as libc::c_int != 0 &&
                           (*this).genericVar2 != 0.0f32 &&
                           (if angleToCollidedActor as libc::c_int >=
                                   0 as libc::c_int {
                                angleToCollidedActor as libc::c_int
                            } else { -(angleToCollidedActor as libc::c_int) })
                               > 0x6000 as libc::c_int {
                    ...
                    if (*this).genericVar2 != 0.0f32 {
                        if (*this).genericVar2 == 65535.0f32 {
                            Audio_PlayActorSound2(thisx,
                                                  0x287d as libc::c_int as
                                                      u16_0);
                        }
                        (*this).genericVar2 =
                            Math_SmoothStepToF(&mut (*thisx).world.pos.z,
                                               (*this).targetPosFwd.z, 1.0f32,
                                               (*this).moveSpeedForwardBack.z,
                                               0.0f32);
                        (*this).genericVar2 +=
                            Math_SmoothStepToF(&mut (*thisx).world.pos.x,
                                               (*this).targetPosFwd.x, 1.0f32,
                                               (*this).moveSpeedForwardBack.x,
                                               0.0f32)
                    }
                }
                16384 => {
                    if (*thisx).params as libc::c_int &
                        ...
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 16313536926714486912;
                    } else if (*thisx).bgCheckFlags as libc::c_int &
                                  8 as libc::c_int != 0 &&
                                  (angleToWall as libc::c_int) <
                                  ...
                                  angleToWall as libc::c_int >
                                      -(0x6000 as libc::c_int) {
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 914440069034635393;
                    } else { current_block_117 = 16313536926714486912; }
                            ...
                            if touchingActor as libc::c_int != 0 &&
                                ...
                                (*this).genericVar2 =
                                    Math_SmoothStepToF(&mut (*thisx).world.pos.x,
                                                       (*this).targetPosLeft.x,
                                                       1.0f32,
                                                       (*this).moveSpeedLeftRight.x,
                                                       0.0f32);
                                (*this).genericVar2 +=
                                    Math_SmoothStepToF(&mut (*thisx).world.pos.z,
                                                       (*this).targetPosLeft.z,
                                                       1.0f32,
                                                       (*this).moveSpeedLeftRight.z,
                                                       0.0f32)
                            }
                        }
                    }
                ...
                -32768 => {
                    if (*thisx).params as libc::c_int &
                           (1 as libc::c_int) << 1 as libc::c_int == 0 {
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 4751196792806374320;
                    } else if (*thisx).bgCheckFlags as libc::c_int &
                                  8 as libc::c_int != 0 &&
                                  (if angleToWall as libc::c_int >=
                                          0 as libc::c_int {
                        ...
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 914440069034635393;
                    } else { current_block_117 = 4751196792806374320; }
                    match current_block_117 {
                        914440069034635393 => { }
                        _ => {
                            if touchingActor as libc::c_int != 0 &&
                ...
                -16384 => {
                    if (*thisx).params as libc::c_int &
                        ...
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 1425453989644512380;
                    } else if (*thisx).bgCheckFlags as libc::c_int &
                                  8 as libc::c_int != 0 &&
                                  angleToWall as libc::c_int >
                                      0x2000 as libc::c_int &&
                        ...
                        (*this).genericVar2 = 0.0f32;
                        current_block_117 = 914440069034635393;
                    } else { current_block_117 = 1425453989644512380; }
                    match current_block_117 {
                        914440069034635393 => { }
                        _ => {
                            if touchingActor as libc::c_int != 0 &&
            ...
            if Actor_TestFloorInDirection(thisx, globalCtx, 50.0f32,
                                          (*this).genericVar1) == 0 {
                (*this).genericVar2 = 0.0f32
            }
            // if in initial position:
        } else if (*thisx).world.pos.x == (*thisx).home.pos.x &&
            ...
            (*this).genericVar1 =
                ((*thisx).yawTowardsPlayer as libc::c_int -
                     (*thisx).world.rot.y as libc::c_int +
                     0x2000 as libc::c_int & 0xc000 as libc::c_int) as s16;
            (*this).genericVar2 = 0.0f32;
            if (*thisx).xzDistToPlayer < 200.0f32 {
                (*this).genericVar2 = 65535.0f32
            }
            ...
            (*this).genericVar1 =
                (Math_Vec3f_Yaw(&mut (*thisx).world.pos,
                                &mut (*thisx).home.pos) as libc::c_int +
                     0x2000 as libc::c_int & 0xc000 as libc::c_int) as s16;
            match (*this).genericVar1 as libc::c_int {
                0 => {
                    // movement is closest to +z direction
                    if (*thisx).bgCheckFlags as libc::c_int & 8 as libc::c_int
                           != 0 {
                        if (if (*thisx).wallYaw as libc::c_int >=
                                   0 as libc::c_int {
                                (*thisx).wallYaw as libc::c_int
                            } else { -((*thisx).wallYaw as libc::c_int) }) >
                               0x6000 as libc::c_int {
                            blockedOnReturn = 1 as libc::c_int as s16
                        }
                    } else if touchingActor as libc::c_int != 0 &&
                                  (if angleToCollidedActor as libc::c_int >=
                                          0 as libc::c_int {
                                       angleToCollidedActor as libc::c_int
                                   } else {
                ...
                16384 => {
                    // movement is closest to +x direction
                    if (*thisx).bgCheckFlags as libc::c_int & 8 as libc::c_int
                           != 0 {
                        if ((*thisx).wallYaw as libc::c_int) <
                               -(0x2000 as libc::c_int) &&
                               (*thisx).wallYaw as libc::c_int >
                                   -(0x6000 as libc::c_int) {
                            blockedOnReturn = 1 as libc::c_int as s16
                ...
                _ => { }
            } // Only used by straight line logic
            if blockedOnReturn == 0 {
                Math_SmoothStepToF(&mut (*thisx).world.pos.x,
                                   (*thisx).home.pos.x, 1.0f32, 3.0f32,
                                   0.0f32);
                Math_SmoothStepToF(&mut (*thisx).world.pos.z,
                                   (*thisx).home.pos.z, 1.0f32, 3.0f32,
                                   0.0f32);
            }
        }
        ...
        Actor_UpdateBgCheckInfo(globalCtx, thisx, 25.0f32, 20.0f32, 20.0f32,
                                0x1d as libc::c_int);
        if (*thisx).params as libc::c_int & 0x10 as libc::c_int != 0 {
...
#[no_mangle]
pub unsafe extern "C" fn EnTrap_Draw(mut thisx: *mut Actor,
                                     mut globalCtx: *mut GlobalContext) {
    func_8002EBCC(thisx, globalCtx, 1 as libc::c_int);
    Gfx_DrawDListOpa(globalCtx, gSlidingBladeTrapDL.as_mut_ptr());
}
