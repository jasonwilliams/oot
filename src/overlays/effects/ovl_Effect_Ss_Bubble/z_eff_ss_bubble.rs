...
#[no_mangle]
pub static mut Effect_Ss_Bubble_InitVars: EffectSsInit =
    unsafe {
        {
            let mut init =
                EffectSsInit{type_0: EFFECT_SS_BUBBLE as libc::c_int as u32_0,
                             init:
                                 Some(EffectSsBubble_Init as
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn EffectSsBubble_Init(mut globalCtx:
                                                 *mut GlobalContext,
                                             mut index: u32_0,
                                             mut this: *mut EffectSs,
                                             mut initParamsx:
                                                 *mut libc::c_void) -> u32_0 {
    let mut initParams: *mut EffectSsBubbleInitParams =
        initParamsx as *mut EffectSsBubbleInitParams;
    ...
    (*this).gfx =
        gSegments[(((if Rand_ZeroOne() < 0.5f32 {
            ...
            as *mut libc::c_void;
    (*this).pos.x =
        (Rand_ZeroOne() - 0.5f32) * (*initParams).xzPosRandScale +
            (*initParams).pos.x;
    (*this).pos.y =
        (Rand_ZeroOne() - 0.5f32) * (*initParams).yPosRandScale +
            (*initParams).yPosOffset + (*initParams).pos.y;
    (*this).pos.z =
        (Rand_ZeroOne() - 0.5f32) * (*initParams).xzPosRandScale +
            (*initParams).pos.z;
    Math_Vec3f_Copy(&mut (*this).vec, &mut (*this).pos);
    (*this).life = 1 as libc::c_int as s16;
    (*this).regs[0 as libc::c_int as usize] =
        ((Rand_ZeroOne() * 0.5f32 + 1.0f32) * (*initParams).scale *
             100 as libc::c_int as libc::c_float) as s16;
    (*this).draw =
        Some(EffectSsBubble_Draw as
                 unsafe extern "C" fn(_: *mut GlobalContext, _: u32_0,
                                      _: *mut EffectSs) -> ());
    (*this).update =
        Some(EffectSsBubble_Update as
                 unsafe extern "C" fn(_: *mut GlobalContext, _: u32_0,
                                      _: *mut EffectSs) -> ());
    return 1 as libc::c_int as u32_0;
}
#[no_mangle]
pub unsafe extern "C" fn EffectSsBubble_Draw(mut globalCtx:
                                                 *mut GlobalContext,
                                             mut index: u32_0,
                                             mut this: *mut EffectSs) {
    ...
    Graph_OpenDisps(dispRefs.as_mut_ptr(), gfxCtx,
                    b"../z_eff_ss_bubble.c\x00" as *const u8 as
...
#[no_mangle]
pub unsafe extern "C" fn EffectSsBubble_Update(mut globalCtx:
                                                   *mut GlobalContext,
                                               mut index: u32_0,
                                               mut this: *mut EffectSs) {
    ...
    if WaterBox_GetSurface1(globalCtx, &mut (*globalCtx).colCtx,
                            (*this).pos.x, (*this).pos.z, &mut waterSurfaceY,
                            &mut waterBox) == 0 {
        (*this).life = -(1 as libc::c_int) as s16;
        return
    }
    if waterSurfaceY < (*this).pos.y {
        ...
        (*this).life += 1;
        (*this).pos.x = Rand_ZeroOne() * 0.5f32 - 0.25f32 + (*this).vec.x;
        (*this).accel.y = (Rand_ZeroOne() - 0.3f32) * 0.2f32;
        (*this).pos.z = Rand_ZeroOne() * 0.5f32 - 0.25f32 + (*this).vec.z
    };
}
