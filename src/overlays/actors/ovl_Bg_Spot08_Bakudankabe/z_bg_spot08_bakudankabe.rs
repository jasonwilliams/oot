...
extern "C" {
    ...
    #[no_mangle]
    static mut gZorasFountainBombableWallDL: [Gfx; 0];
    #[no_mangle]
    static mut gZorasFountainBombableWallCol: CollisionHeader;
    #[no_mangle]
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct BgSpot08Bakudankabe {
    pub dyna: DynaPolyActor,
    pub collider: ColliderJntSph,
    pub colliderItems: [ColliderJntSphElement; 3],
}
...
#[no_mangle]
pub static mut Bg_Spot08_Bakudankabe_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id:
                              ACTOR_BG_SPOT08_BAKUDANKABE as libc::c_int as
                                  s16,
                          ...
                          instanceSize:
                              ::std::mem::size_of::<BgSpot08Bakudankabe>() as
                                  libc::c_ulong,
                          init:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(BgSpot08Bakudankabe_Init
                                                                          as
                          ...
                          destroy:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(BgSpot08Bakudankabe_Destroy
                                                                          as
                                                                          unsafe extern "C" fn(_:
                                                                                                   *mut Actor,
                                                                                               _:
                          ...
                          update:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(BgSpot08Bakudankabe_Update
                                                                          as
                                                                          unsafe extern "C" fn(_:
                                                                                                   *mut Actor,
                                                                                               _:
                          ...
                          draw:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(BgSpot08Bakudankabe_Draw
                                                                          as
                                                                          unsafe extern "C" fn(_:
                                                                                                   *mut Actor,
                                                                                               _:
...
static mut sJntSphElementsInit: [ColliderJntSphElementInit; 3] =
    [{
         let mut init =
...
static mut D_808B08AC: [Vec3f; 3] =
    [{ let mut init = Vec3f{x: 0.0f32, y: 116.65f32, z: 50.0f32,}; init },
     { let mut init = Vec3f{x: 115.0f32, y: 95.0f32, z: 10.0f32,}; init },
     { let mut init = Vec3f{x: -115.0f32, y: 95.0f32, z: 10.0f32,}; init }];
// Initialized in run_static_initializers
static mut sInitChain: [InitChainEntry; 4] =
    [InitChainEntry{cont_type_0_offset_value: [0; 4],}; 4];
#[no_mangle]
pub unsafe extern "C" fn func_808B02D0(mut this: *mut BgSpot08Bakudankabe,
...
#[no_mangle]
pub unsafe extern "C" fn func_808B0324(mut this: *mut BgSpot08Bakudankabe,
                                       mut globalCtx: *mut GlobalContext) {
    let mut pad: [s32; 2] = [0; 2];
    let mut i: s32 = 0;
    let mut burstDepthY: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut burstDepthX: Vec3f = Vec3f{x: 0., y: 0., z: 0.,};
    let mut sinY: f32_0 = 0.;
    let mut cosY: f32_0 = 0.;
    sinY = Math_SinS((*this).dyna.actor.shape.rot.y);
    cosY = Math_CosS((*this).dyna.actor.shape.rot.y);
    burstDepthX.z = 0.0f32;
    burstDepthX.x = 0.0f32;
    i = 0 as libc::c_int;
    while i < 24 as libc::c_int {
        let mut gravityInfluence: s16 = 0;
        let mut scale: s16 = 0;
        let mut temp1: f32_0 = 0.;
        let mut temp2: f32_0 = 0.;
        let mut rotationSpeed: s32 = 0;
        temp1 = (Rand_ZeroOne() - 0.5f32) * 440.0f32;
        temp2 = (Rand_ZeroOne() - 0.5f32) * 20.0f32;
        burstDepthY.x =
            (*this).dyna.actor.world.pos.x + temp2 * sinY + temp1 * cosY;
        burstDepthY.y =
            (*this).dyna.actor.world.pos.y + 20.0f32 +
                i as libc::c_float * (65.0f32 / 12.0f32);
        burstDepthY.z =
            (*this).dyna.actor.world.pos.z + temp2 * cosY - temp1 * sinY;
        burstDepthX.y = (Rand_ZeroOne() - 0.2f32) * 12.0f32;
        scale = (Rand_ZeroOne() * 75.0f32 + 10.0f32) as s16;
        if (scale as libc::c_int) < 25 as libc::c_int {
            gravityInfluence = -(300 as libc::c_int) as s16
        } else if (scale as libc::c_int) < 50 as libc::c_int {
            gravityInfluence = -(360 as libc::c_int) as s16
        } else { gravityInfluence = -(420 as libc::c_int) as s16 }
        if Rand_ZeroOne() < 0.4f32 {
            rotationSpeed = 65 as libc::c_int
        } else { rotationSpeed = 33 as libc::c_int }
        EffectSsKakera_Spawn(globalCtx, &mut burstDepthY, &mut burstDepthX,
                             &mut burstDepthY, gravityInfluence,
                             rotationSpeed as s16, 0x1e as libc::c_int as s16,
                             4 as libc::c_int as s16, 0 as libc::c_int as s16,
                             scale, 1 as libc::c_int as s16,
                             3 as libc::c_int as s16, 80 as libc::c_int,
    ...
    while i <
        ...
        burstDepthY.x =
            (*this).dyna.actor.world.pos.x + D_808B08AC[i as usize].z * sinY +
                D_808B08AC[i as usize].x * cosY;
        burstDepthY.y =
            (*this).dyna.actor.world.pos.y + D_808B08AC[i as usize].y;
        burstDepthY.z =
            (*this).dyna.actor.world.pos.z + D_808B08AC[i as usize].z * cosY -
                D_808B08AC[i as usize].x * sinY;
        func_80033480(globalCtx, &mut burstDepthY, 120.0f32, 4 as libc::c_int,
                      0x78 as libc::c_int as s16, 0xa0 as libc::c_int as s16,
                      1 as libc::c_int as u8_0);
        i += 1
    };
}
#[no_mangle]
pub unsafe extern "C" fn BgSpot08Bakudankabe_Init(mut thisx: *mut Actor,
                                                  mut globalCtx:
                                                      *mut GlobalContext) {
    let mut this: *mut BgSpot08Bakudankabe =
        thisx as *mut BgSpot08Bakudankabe;
    let mut pad: s32 = 0;
    ...
    CollisionHeader_GetVirtual(&mut gZorasFountainBombableWallCol as
...
#[no_mangle]
pub unsafe extern "C" fn BgSpot08Bakudankabe_Destroy(mut thisx: *mut Actor,
                                                     mut globalCtx:
                                                         *mut GlobalContext) {
    let mut this: *mut BgSpot08Bakudankabe =
        thisx as *mut BgSpot08Bakudankabe;
...
#[no_mangle]
pub unsafe extern "C" fn BgSpot08Bakudankabe_Update(mut thisx: *mut Actor,
                                                    mut globalCtx:
                                                        *mut GlobalContext) {
    let mut this: *mut BgSpot08Bakudankabe =
        thisx as *mut BgSpot08Bakudankabe;
    if (*this).collider.base.acFlags as libc::c_int &
        ...
        Actor_Kill(&mut (*this).dyna.actor);
    } else if (*this).dyna.actor.xzDistToPlayer < 800.0f32 {
        CollisionCheck_SetAC(globalCtx, &mut (*globalCtx).colChkCtx,
                             &mut (*this).collider.base);
    };
}
#[no_mangle]
pub unsafe extern "C" fn BgSpot08Bakudankabe_Draw(mut thisx: *mut Actor,
                                                  mut globalCtx:
                                                      *mut GlobalContext) {
    let mut this: *mut BgSpot08Bakudankabe =
        thisx as *mut BgSpot08Bakudankabe;
    ...
    Gfx_DrawDListOpa(globalCtx, gZorasFountainBombableWallDL.as_mut_ptr());
}
