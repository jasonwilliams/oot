...
extern "C" {
    ...
    #[no_mangle]
    static mut object_dh_Skel_000BD8: FlexSkeletonHeader;
    #[no_mangle]
    static mut object_dh_Anim_0015B0: AnimationHeader;
}
...
pub struct EnDha {
    pub actor: Actor,
    pub skelAnime: SkelAnime,
    pub jointTable: [Vec3s; 4],
    pub morphTable: [Vec3s; 4],
    pub unk_1C0: u8_0,
    pub actionFunc: EnDhaActionFunc,
    pub actionTimer: s16,
    pub timer: s16,
    pub unk_1CC: u8_0,
    pub limbAngleX: [s16; 2],
    pub limbAngleY: s16,
    pub handAngle: Vec3s,
    pub handPos: [Vec3f; 2],
    pub armPos: Vec3f,
    pub collider: ColliderJntSph,
    pub colliderItem: [ColliderJntSphElement; 5],
}
pub type EnDhaActionFunc
    =
    Option<unsafe extern "C" fn(_: *mut EnDha, _: *mut GlobalContext) -> ()>;
pub type C2RustUnnamed_22 = libc::c_int;
pub const ENDH_DEATH: C2RustUnnamed_22 = 10;
pub const ENDH_START_ATTACK_BOMB: C2RustUnnamed_22 = 5;
pub const ENDH_START_ATTACK_GRAB: C2RustUnnamed_22 = 1;
pub const ENDH_WAIT_UNDERGROUND: C2RustUnnamed_22 = 0;
pub const ENDH_HANDS_KILLED_1: C2RustUnnamed_22 = -1;
pub const ENDH_HANDS_KILLED_2: C2RustUnnamed_22 = -2;
pub const ENDH_HANDS_KILLED_3: C2RustUnnamed_22 = -3;
pub const ENDH_HANDS_KILLED_4: C2RustUnnamed_22 = -4;
#[no_mangle]
pub static mut En_Dha_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id: ACTOR_EN_DHA as libc::c_int as s16,
                          ...
                          instanceSize:
                              ::std::mem::size_of::<EnDha>() as libc::c_ulong,
                          init:
...
static mut sJntSphElementsInit: [ColliderJntSphElementInit; 5] =
    [{
         let mut init =
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_SetupAction(mut this: *mut EnDha,
                                           mut actionFunc: EnDhaActionFunc) {
    (*this).actionFunc = actionFunc; // unused
}
#[no_mangle]
pub unsafe extern "C" fn EnDha_Init(mut thisx: *mut Actor,
                                    mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnDha = thisx as *mut EnDha;
    ...
    SkelAnime_InitFlex(globalCtx, &mut (*this).skelAnime,
                       &mut object_dh_Skel_000BD8, &mut object_dh_Anim_0015B0,
                       (*this).jointTable.as_mut_ptr(),
                       (*this).morphTable.as_mut_ptr(), 4 as libc::c_int);
    ...
    (*this).actor.colChkInfo.health = 8 as libc::c_int as u8_0;
    (*this).limbAngleX[0 as libc::c_int as usize] =
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_Destroy(mut thisx: *mut Actor,
                                       mut globalCtx: *mut GlobalContext) {
    let mut pad: s32 = 0;
    let mut this: *mut EnDha = thisx as *mut EnDha;
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_SetupWait(mut this: *mut EnDha) {
    Animation_PlayLoop(&mut (*this).skelAnime, &mut object_dh_Anim_0015B0);
    (*this).unk_1C0 = 0 as libc::c_int as u8_0;
    (*this).actionTimer = (Rand_ZeroOne() * 10.0f32 + 5.0f32) as s16;
    (*this).actor.speedXZ = 0.0f32;
    (*this).actor.world.rot.y = (*this).actor.shape.rot.y;
    (*this).actor.home.rot.z = 1 as libc::c_int as s16;
    EnDha_SetupAction(this,
                      Some(EnDha_Wait as
                               unsafe extern "C" fn(_: *mut EnDha,
                                                    _: *mut GlobalContext)
                                   -> ()));
}
#[no_mangle]
pub unsafe extern "C" fn EnDha_Wait(mut this: *mut EnDha,
                                    mut globalCtx: *mut GlobalContext) {
    let mut zeroVec: Vec3f =
        { let mut init = Vec3f{x: 0.0f32, y: 0.0f32, z: 0.0f32,}; init };
    let mut armPosMultiplier1: Vec3f =
        { let mut init = Vec3f{x: 0.0f32, y: 0.0f32, z: 55.0f32,}; init };
    let mut armPosMultiplier2: Vec3f =
        { let mut init = Vec3f{x: 0.0f32, y: 0.0f32, z: -54.0f32,}; init };
    let mut player: *mut Player =
        (*globalCtx).actorCtx.actorLists[ACTORCAT_PLAYER as libc::c_int as
                                             usize].head as *mut Player;
    let mut pad: s32 = 0;
    let mut pad2: s32 = 0;
    let mut playerPos: Vec3f = (*player).actor.world.pos;
    let mut angle: Vec3s = Vec3s{x: 0, y: 0, z: 0,};
    let mut yaw: s16 = 0;
    playerPos.x += Math_SinS((*player).actor.shape.rot.y) * -5.0f32;
    playerPos.z += Math_CosS((*player).actor.shape.rot.y) * -5.0f32;
    if !(gSaveContext.linkAge == 0 as libc::c_int) {
        playerPos.y += 38.0f32
    } else { playerPos.y += 56.0f32 }
    if (*this).actor.xzDistToPlayer <= 100.0f32 {
        (*this).limbAngleY = 0 as libc::c_int as s16;
        (*this).handAngle.x = (*this).limbAngleY;
        (*this).handAngle.y = (*this).handAngle.x;
        if Math_Vec3f_DistXYZ(&mut playerPos,
                              &mut *(*this).handPos.as_mut_ptr().offset(0 as
                                                                            libc::c_int
                                                                            as
                                                                            isize))
               <= 12.0f32 {
            if (*this).unk_1CC as libc::c_int == 0 as libc::c_int {
                if (*globalCtx).grabPlayer.expect("non-null function pointer")(globalCtx,
                    ...
                    (*this).unk_1CC = (*this).unk_1CC.wrapping_add(1);
                    if !(*this).actor.parent.is_null() {
                        (*(*this).actor.parent).params =
                            ENDH_START_ATTACK_GRAB as libc::c_int as s16
                    }
                    Audio_PlayActorSound2(&mut (*this).actor,
                                          0x3902 as libc::c_int as u16_0);
                }
            } else {
                (*this).timer =
                    ((*this).timer as libc::c_int + 0x1194 as libc::c_int) as
                        s16;
                (*this).limbAngleY =
                    (Math_SinS((*this).timer) * 1820.0f32) as s16;
                if (*player).stateFlags2 & 0x80 as libc::c_int as libc::c_uint
                       == 0 {
                    (*this).unk_1CC = 0 as libc::c_int as u8_0;
                    EnDha_SetupTakeDamage(this);
                    return
                }
                if ((*this).timer as libc::c_int) < -(0x6e6b as libc::c_int) {
                    Audio_PlayActorSound2(&mut (*this).actor,
                                          0x3902 as libc::c_int as u16_0);
                }
            }
            func_80035844(&mut *(*this).handPos.as_mut_ptr().offset(1 as
                                                                        libc::c_int
                                                                        as
                                                                        isize),
                          &mut playerPos, &mut (*this).handAngle,
                          0 as libc::c_int);
            (*this).handAngle.y =
                ((*this).handAngle.y as libc::c_int -
                     ((*this).actor.shape.rot.y as libc::c_int +
                          (*this).limbAngleY as libc::c_int)) as s16;
            (*this).handAngle.x =
                ((*this).handAngle.x as libc::c_int -
                     ((*this).actor.shape.rot.x as libc::c_int +
                          (*this).limbAngleX[0 as libc::c_int as usize] as
                              libc::c_int +
                          (*this).limbAngleX[1 as libc::c_int as usize] as
                              libc::c_int)) as s16
        } else {
            if (*player).stateFlags2 & 0x80 as libc::c_int as libc::c_uint !=
                   0 &&
                   &mut (*this).actor as *mut Actor == (*player).actor.parent
               {
                (*player).stateFlags2 &=
            ...
            if (*this).actor.home.rot.z as libc::c_int != 0 as libc::c_int {
                Audio_PlayActorSound2(&mut (*this).actor,
                                      0x3903 as libc::c_int as u16_0);
                (*this).actor.home.rot.z = 0 as libc::c_int as s16
            }
        }
        (*this).actor.shape.rot.y =
            Math_Vec3f_Yaw(&mut (*this).actor.world.pos, &mut playerPos);
        Math_SmoothStepToF(&mut (*(*this).handPos.as_mut_ptr().offset(0 as
                                                                          libc::c_int
                                                                          as
                                                                          isize)).x,
                           playerPos.x, 1.0f32, 16.0f32, 0.0f32);
        Math_SmoothStepToF(&mut (*(*this).handPos.as_mut_ptr().offset(0 as
                                                                          libc::c_int
                                                                          as
                                                                          isize)).y,
                           playerPos.y, 1.0f32, 16.0f32, 0.0f32);
        Math_SmoothStepToF(&mut (*(*this).handPos.as_mut_ptr().offset(0 as
                                                                          libc::c_int
                                                                          as
                                                                          isize)).z,
                           playerPos.z, 1.0f32, 16.0f32, 0.0f32);
        func_80035844(&mut (*this).armPos,
                      &mut *(*this).handPos.as_mut_ptr().offset(0 as
                                                                    libc::c_int
                                                                    as isize),
                      &mut angle, 0 as libc::c_int);
        Matrix_Translate((*this).handPos[0 as libc::c_int as usize].x,
                         (*this).handPos[0 as libc::c_int as usize].y,
                         (*this).handPos[0 as libc::c_int as usize].z,
                         MTXMODE_NEW as libc::c_int as u8_0);
        Matrix_RotateZYX(angle.x, angle.y, 0 as libc::c_int as s16,
                         MTXMODE_APPLY as libc::c_int as u8_0);
        Matrix_MultVec3f(&mut armPosMultiplier2, &mut (*this).armPos);
        Matrix_Translate((*this).actor.world.pos.x, (*this).actor.world.pos.y,
                         (*this).actor.world.pos.z,
                         MTXMODE_NEW as libc::c_int as u8_0);
        func_80035844(&mut (*this).actor.world.pos, &mut (*this).armPos,
                      &mut angle, 0 as libc::c_int);
        Matrix_RotateZYX(angle.x, angle.y, 0 as libc::c_int as s16,
                         MTXMODE_APPLY as libc::c_int as u8_0);
        Matrix_MultVec3f(&mut armPosMultiplier1, &mut (*this).armPos);
        (*this).limbAngleX[0 as libc::c_int as usize] =
            Math_Vec3f_Pitch(&mut (*this).actor.world.pos,
                             &mut (*this).armPos);
        yaw =
            (Math_Vec3f_Yaw(&mut (*this).actor.world.pos, &mut (*this).armPos)
                 as libc::c_int - (*this).actor.shape.rot.y as libc::c_int) as
                s16;
        if (if yaw as libc::c_int >= 0 as libc::c_int {
                yaw as libc::c_int
            } else { -(yaw as libc::c_int) }) >= 0x4000 as libc::c_int {
            (*this).limbAngleX[0 as libc::c_int as usize] =
                (-(0x8000 as libc::c_int) -
                     (*this).limbAngleX[0 as libc::c_int as usize] as
                         libc::c_int) as s16
        }
        (*this).limbAngleX[1 as libc::c_int as usize] =
            (Math_Vec3f_Pitch(&mut (*this).armPos,
                              &mut *(*this).handPos.as_mut_ptr().offset(0 as
                                                                            libc::c_int
                                                                            as
                                                                            isize))
                 as libc::c_int -
                 (*this).limbAngleX[0 as libc::c_int as usize] as libc::c_int)
                as s16;
        if ((*this).limbAngleX[1 as libc::c_int as usize] as libc::c_int) <
               0 as libc::c_int {
            (*this).limbAngleX[0 as libc::c_int as usize] =
                ((*this).limbAngleX[0 as libc::c_int as usize] as libc::c_int
                     +
            ...
            (*this).limbAngleX[1 as libc::c_int as usize] =
                ((*this).limbAngleX[1 as libc::c_int as usize] as libc::c_int
        ...
        (*this).actor.home.rot.z = 1 as libc::c_int as s16;
        Math_SmoothStepToS(&mut *(*this).limbAngleX.as_mut_ptr().offset(1 as
                                                                            libc::c_int
                                                                            as
                                                                            isize),
        ...
        Math_SmoothStepToS(&mut *(*this).limbAngleX.as_mut_ptr().offset(0 as
                                                                            libc::c_int
                                                                            as
                                                                            isize),
                           -(0x4000 as libc::c_int) as s16,
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_SetupTakeDamage(mut this: *mut EnDha) {
    (*this).actionTimer = 15 as libc::c_int as s16;
    EnDha_SetupAction(this,
                      Some(EnDha_TakeDamage as
                               unsafe extern "C" fn(_: *mut EnDha,
                                                    _: *mut GlobalContext)
                                   -> ()));
}
#[no_mangle]
pub unsafe extern "C" fn EnDha_TakeDamage(mut this: *mut EnDha,
                                          mut globalCtx: *mut GlobalContext) {
    let mut player: *mut Player =
        (*globalCtx).actorCtx.actorLists[ACTORCAT_PLAYER as libc::c_int as
                                             usize].head as *mut Player;
    ...
    Math_SmoothStepToS(&mut (*this).limbAngleY, 0 as libc::c_int as s16,
    ...
    Math_SmoothStepToS(&mut *(*this).limbAngleX.as_mut_ptr().offset(0 as
                                                                        ...
                                                                        isize),
                       -(0x4000 as libc::c_int) as s16,
                       1 as libc::c_int as s16, 2000 as libc::c_int as s16,
                       0 as libc::c_int as s16);
    SkelAnime_Update(&mut (*this).skelAnime);
    (*this).actionTimer -= 1;
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_SetupDeath(mut this: *mut EnDha) {
    (*this).unk_1C0 = 8 as libc::c_int as u8_0;
    (*this).actionTimer = 300 as libc::c_int as s16;
    if !(*this).actor.parent.is_null() {
        if (*(*this).actor.parent).params as libc::c_int !=
               ENDH_DEATH as libc::c_int {
            Audio_PlayActorSound2(&mut (*this).actor,
                                  0x39aa as libc::c_int as u16_0);
        }
        if (*(*this).actor.parent).params as libc::c_int <=
               ENDH_WAIT_UNDERGROUND as libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_Die(mut this: *mut EnDha,
                                   mut globalCtx: *mut GlobalContext) {
    let mut angle: s16 = 0;
    ...
    Math_SmoothStepToS(&mut *(*this).limbAngleX.as_mut_ptr().offset(1 as
                                                                        ...
                                                                        isize),
                       0 as libc::c_int as s16, 1 as libc::c_int as s16,
                       0x7d0 as libc::c_int as s16, 0 as libc::c_int as s16);
    angle =
        Math_SmoothStepToS(&mut *(*this).limbAngleX.as_mut_ptr().offset(0 as
                                                                            libc::c_int
                                                                            as
    ...
    if angle as libc::c_int == 0 as libc::c_int {
        ...
        if (*this).actionTimer as libc::c_int != 0 as libc::c_int {
            if -12000.0f32 < (*this).actor.shape.yOffset {
                (*this).actor.shape.yOffset -= 1000.0f32;
                func_80033480(globalCtx, &mut vec, 7.0f32, 1 as libc::c_int,
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_UpdateHealth(mut this: *mut EnDha,
                                            mut globalCtx:
                                                *mut GlobalContext) {
    if !((*this).unk_1C0 as libc::c_int >= 8 as libc::c_int ||
        ...
        if (*this).actor.colChkInfo.damageEffect as libc::c_int ==
            ...
            if Actor_ApplyDamage(&mut (*this).actor) as libc::c_int ==
                ...
                Audio_PlayActorSound2(&mut (*this).actor,
                                      0x39a9 as libc::c_int as u16_0);
                (*this).unk_1C0 = 9 as libc::c_int as u8_0;
                EnDha_SetupTakeDamage(this);
            }
        }
    }
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_Update(mut thisx: *mut Actor,
                                      mut globalCtx: *mut GlobalContext) {
    let mut pad: s32 = 0;
    let mut this: *mut EnDha = thisx as *mut EnDha;
    if (*this).actor.parent.is_null() {
        (*this).actor.parent =
            Actor_FindNearby(globalCtx, &mut (*this).actor,
                             ACTOR_EN_DH as libc::c_int as s16,
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_OverrideLimbDraw(mut globalCtx:
                                                    *mut GlobalContext,
                                                mut limbIndex: s32,
 ...
 -> s32 {
    ...
    if limbIndex == 1 as libc::c_int {
        (*rot).y =
            -(((*this).limbAngleX[0 as libc::c_int as usize] as libc::c_int +
                   0x4000 as libc::c_int) as s16 as libc::c_int) as s16;
        (*rot).z =
            ((*rot).z as libc::c_int + (*this).limbAngleY as libc::c_int) as
                s16
    } else if limbIndex == 2 as libc::c_int {
        (*rot).z = (*this).limbAngleX[1 as libc::c_int as usize];
        (*rot).y =
            ((*rot).y as libc::c_int - (*this).limbAngleY as libc::c_int) as
                s16
    } else if limbIndex == 3 as libc::c_int {
        (*rot).y = -((*this).handAngle.y as libc::c_int) as s16;
        (*rot).z = -((*this).handAngle.x as libc::c_int) as s16
    }
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn EnDha_OverridePostDraw(mut globalCtx:
                                                    *mut GlobalContext,
                                                mut limbIndex: s32,
                                                mut dList: *mut *mut Gfx,
                                                mut rot: *mut Vec3s,
                                                mut thisx:
                                                    *mut libc::c_void) {
    let mut handVec: Vec3f =
        { let mut init = Vec3f{x: 1100.0f32, y: 0.0f32, z: 0.0f32,}; init };
    let mut zeroVec: Vec3f =
        { let mut init = Vec3f{x: 0.0f32, y: 0.0f32, z: 0.0f32,}; init };
    let mut this: *mut EnDha = thisx as *mut EnDha;
    match limbIndex {
        1 => {
            Collider_UpdateSpheres(2 as libc::c_int, &mut (*this).collider);
            Collider_UpdateSpheres(3 as libc::c_int, &mut (*this).collider);
        }
        2 => {
            Collider_UpdateSpheres(4 as libc::c_int, &mut (*this).collider);
            Collider_UpdateSpheres(5 as libc::c_int, &mut (*this).collider);
            Matrix_MultVec3f(&mut zeroVec, &mut (*this).armPos);
        }
        3 => {
            Collider_UpdateSpheres(1 as libc::c_int, &mut (*this).collider);
            Matrix_MultVec3f(&mut handVec,
                             &mut *(*this).handPos.as_mut_ptr().offset(0 as
                                                                           libc::c_int
                                                                           as
                                                                           isize));
            Matrix_MultVec3f(&mut zeroVec,
                             &mut *(*this).handPos.as_mut_ptr().offset(1 as
                                                                           libc::c_int
                                                                           as
                                                                           isize));
        }
...
#[no_mangle]
pub unsafe extern "C" fn EnDha_Draw(mut thisx: *mut Actor,
                                    mut globalCtx: *mut GlobalContext) {
    let mut pad: s32 = 0;
    let mut this: *mut EnDha = thisx as *mut EnDha;
    ...
    SkelAnime_DrawFlexOpa(globalCtx, (*this).skelAnime.skeleton,
                          ...
                          Some(EnDha_OverrideLimbDraw as
                                   unsafe extern "C" fn(_: *mut GlobalContext,
                                                        ...
                                                        _: *mut libc::c_void)
                                       -> s32),
                          Some(EnDha_OverridePostDraw as
                                   unsafe extern "C" fn(_: *mut GlobalContext,
                                                        _: s32,
                                                        _: *mut *mut Gfx,
                                                        _: *mut Vec3s,
...
unsafe extern "C" fn run_static_initializers() {
    sInitChain =
        [{
             ...
             init.set_value(0x2e as libc::c_int);
             init
         },
         {
             let mut init = InitChainEntry{cont_type_0_offset_value: [0; 4],};
