...
extern "C" {
    ...
    #[no_mangle]
    static mut gUnusedCandleDL: [Gfx; 0];
}
...
#[no_mangle]
pub static mut En_Light_InitVars: ActorInit =
    unsafe {
        {
            let mut init =
                ActorInit{id: ACTOR_EN_LIGHT as libc::c_int as s16,
                          category:
                          ...
                          destroy:
                              ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                  ...
                                                                                  _:
                                                                                      *mut GlobalContext)
                                                                 -> ()>,
                                                      ActorFunc>(Some(EnLight_Destroy
                                                                          as
                                                                          unsafe extern "C" fn(_:
                                                                                                   *mut Actor,
                                                                                               _:
...
static mut D_80A9E840: [FlameParams; 16] =
    [{
         let mut init =
             FlameParams{primColor:
                             {
                                 let mut init =
                                     Color_RGBA8{r:
                         ...
                         scale: 75 as libc::c_int as u8_0,};
         init
     },
     {
         let mut init =
     ...
     },
     {
         let mut init =
             FlameParams{primColor:
                         ...
                         scale: 40 as libc::c_int as u8_0,};
         init
     },
     {
         let mut init =
...
pub unsafe extern "C" fn EnLight_Init(mut thisx: *mut Actor,
    ...
    Actor_SetScale(&mut (*this).actor,
                   D_80A9E840[((*this).actor.params as libc::c_int &
                                   0xf as libc::c_int) as usize].scale as
                       libc::c_int as libc::c_float * 0.0001f32);
    (*this).timer = (Rand_ZeroOne() * 255.0f32) as s32 as u8_0;
    if (*this).actor.params as libc::c_int & 0x400 as libc::c_int !=
           0 as libc::c_int {
        (*this).actor.update =
            Some(EnLight_UpdateSwitch as
                     unsafe extern "C" fn(_: *mut Actor,
                                          _: *mut GlobalContext) -> ())
    };
}
#[no_mangle]
pub unsafe extern "C" fn EnLight_Destroy(mut thisx: *mut Actor,
                                         mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnLight = thisx as *mut EnLight;
...
#[no_mangle]
pub unsafe extern "C" fn EnLight_UpdatePosRot(mut this: *mut EnLight,
                                              mut globalCtx:
                                                  *mut GlobalContext) {
    ...
    if !(*this).actor.parent.is_null() {
        ...
        (*this).actor.world.pos.y += 17.0f32
    }
    (*this).timer = (*this).timer.wrapping_add(1);
}
#[no_mangle]
pub unsafe extern "C" fn EnLight_Update(mut thisx: *mut Actor,
                                        mut globalCtx: *mut GlobalContext) {
    let mut intensity: f32_0 = 0.;
    let mut flameParams: *mut FlameParams = 0 as *mut FlameParams;
    let mut radius: s16 = 0;
    let mut this: *mut EnLight = thisx as *mut EnLight;
    flameParams =
        &mut *D_80A9E840.as_mut_ptr().offset(((*this).actor.params as
                                                  libc::c_int &
                                                  0xf as libc::c_int) as
                                                 isize) as *mut FlameParams;
    intensity = Rand_ZeroOne() * 0.5f32 + 0.5f32;
    radius =
        if ((*this).actor.params as libc::c_int) < 0 as libc::c_int {
            100 as libc::c_int
        } else { 300 as libc::c_int } as s16;
    Lights_PointSetColorAndRadius(&mut (*this).lightInfo,
                                  ((*flameParams).primColor.r as libc::c_int
                                       as libc::c_float * intensity) as u8_0,
                                  ((*flameParams).primColor.g as libc::c_int
                                       as libc::c_float * intensity) as u8_0,
                                  ((*flameParams).primColor.b as libc::c_int
                                       as libc::c_float * intensity) as u8_0,
                                  radius);
    EnLight_UpdatePosRot(this, globalCtx);
    if (*this).actor.params as libc::c_int >= 0 as libc::c_int {
        Audio_PlayActorSound2(&mut (*this).actor,
                              (0x2831 as libc::c_int - 0x800 as libc::c_int)
                                  as u16_0);
    };
}
#[no_mangle]
pub unsafe extern "C" fn EnLight_UpdateSwitch(mut thisx: *mut Actor,
                                              mut globalCtx:
                                                  *mut GlobalContext) {
    let mut intensity: f32_0 = 0.;
    let mut flameParams: *mut FlameParams = 0 as *mut FlameParams;
    ...
    scale =
        ((*this).actor.scale.x as libc::c_double /
             ((*flameParams).scale as f32_0 as libc::c_double * 0.0001f64)) as
            f32_0;
    if (*this).actor.params as libc::c_int & 0x800 as libc::c_int !=
           0 as libc::c_int {
        if Flags_GetSwitch(globalCtx,
                           ((*this).actor.params as libc::c_int &
                                0x3f0 as libc::c_int) >> 4 as libc::c_int) !=
               0 {
            Math_StepToF(&mut scale, 1.0f32, 0.05f32);
        } else {
            if scale < 0.1f32 {
                Actor_SetScale(&mut (*this).actor, 0.0f32);
                return
            }
            Math_StepToF(&mut scale, 0.0f32, 0.05f32);
        }
        ...
        Math_StepToF(&mut scale, 0.0f32, 0.05f32);
    } else { Math_StepToF(&mut scale, 1.0f32, 0.05f32); }
    Actor_SetScale(&mut (*this).actor,
                   ((*flameParams).scale as f32_0 as libc::c_double *
                        0.0001f64 * scale as libc::c_double) as f32_0);
    intensity = Rand_ZeroOne() * 0.5f32 + 0.5f32;
    Lights_PointSetColorAndRadius(&mut (*this).lightInfo,
                                  ((*flameParams).primColor.r as libc::c_int
                                       as libc::c_float * intensity) as u8_0,
...
#[no_mangle]
pub unsafe extern "C" fn EnLight_Draw(mut thisx: *mut Actor,
                                      mut globalCtx: *mut GlobalContext) {
    let mut this: *mut EnLight = thisx as *mut EnLight;
    let mut pad: s32 = 0;
    let mut flameParams: *mut FlameParams = 0 as *mut FlameParams;
    ...
    Graph_OpenDisps(dispRefs.as_mut_ptr(), (*globalCtx).state.gfxCtx,
                    b"../z_en_light.c\x00" as *const u8 as
    ...
    if (*this).actor.params as libc::c_int >= 0 as libc::c_int {
        ...
        (*_g).words.w1 =
            Gfx_TwoTexScroll((*globalCtx).state.gfxCtx, 0 as libc::c_int,
                             ...
                             ((*this).timer as libc::c_int *
                                  -(20 as libc::c_int) & 511 as libc::c_int)
                                 as u32_0, 32 as libc::c_int,
                             128 as libc::c_int) as libc::c_uint;
        dList = gEffFire1DL.as_mut_ptr();
        ...
        (*_g_0).words.w0 =
            (0xfa as libc::c_int as u32_0 &
                ...
                (0x80 as libc::c_int as u32_0 &
                    ...
                    0 as libc::c_int;
        (*_g_0).words.w1 =
            ((*flameParams).primColor.r as u32_0 &
                 (((0x1 as libc::c_int) << 8 as libc::c_int) -
                      1 as libc::c_int) as libc::c_uint) << 24 as libc::c_int
                |
                ((*flameParams).primColor.g as u32_0 &
                     (((0x1 as libc::c_int) << 8 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    16 as libc::c_int |
                ((*flameParams).primColor.b as u32_0 &
                     (((0x1 as libc::c_int) << 8 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    8 as libc::c_int |
                ((*flameParams).primColor.a as u32_0 &
                     (((0x1 as libc::c_int) << 8 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    0 as libc::c_int;
        ...
        (*_g_1).words.w0 =
            (0xfb as libc::c_int as u32_0 &
                 (((0x1 as libc::c_int) << 8 as libc::c_int) -
                      1 as libc::c_int) as libc::c_uint) << 24 as libc::c_int;
        (*_g_1).words.w1 =
            ((*flameParams).envColor.r as u32_0 &
                 (((0x1 as libc::c_int) << 8 as libc::c_int) -
                      1 as libc::c_int) as libc::c_uint) << 24 as libc::c_int
                |
                ((*flameParams).envColor.g as u32_0 &
                     (((0x1 as libc::c_int) << 8 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    16 as libc::c_int |
                ((*flameParams).envColor.b as u32_0 &
                     (((0x1 as libc::c_int) << 8 as libc::c_int) -
                          1 as libc::c_int) as libc::c_uint) <<
                    8 as libc::c_int |
                (0 as libc::c_int as u32_0 &
        ...
        (*_g_2).words.w1 =
            Gfx_TwoTexScroll((*globalCtx).state.gfxCtx, 0 as libc::c_int,
                             ...
                             ((*this).timer as libc::c_int *
                                  -(6 as libc::c_int) &
                                  127 as libc::c_int * 1 as libc::c_int) as
                                 u32_0, 16 as libc::c_int, 32 as libc::c_int)
                as libc::c_uint;
        dList = gUnusedCandleDL.as_mut_ptr();
